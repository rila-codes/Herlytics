    ...
    <div className="glass rounded-[2.5rem] border border-white/50 shadow-card p-6 md:p-8 space-y-6 bg-gradient-to-br from-white via-purple-50/30 to-pink-50/40">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3.5 bg-blue-50/70 border border-blue-200 rounded-2xl space-y-1">
          ...
          <span className="text-base font-black text-blue-900 block">24 / 30 Days</span>
          <span className="text-[9px] text-blue-700 font-bold block">80% Monthly Consistency</span>
        </div>

        <div className="p-3.5 bg-purple-50/70 border border-purple-200 rounded-2xl space-y-1">
          <span className="text-[10px] font-black text-purple-600 uppercase block">Sleep Average</span>
          <span className="text-base font-black text-purple-900 block">7.6 Hours</span>
          <span className="text-[9px] text-purple-700 font-bold block">Optimum Sleep Hygiene</span>
        </div>

        <div className="p-3.5 bg-emerald-50/70 border border-emerald-200 rounded-2xl space-y-1">
          <span className="text-[10px] font-black text-emerald-600 uppercase block">Active Movement</span>
          <span className="text-base font-black text-emerald-900 block">16 Sessions</span>
          <span className="text-[9px] text-emerald-700 font-bold block">Cortisol Management</span>
        </div>

        <div className="p-3.5 bg-amber-50/70 border border-amber-200 rounded-2xl space-y-1">
          <span className="text-[10px] font-black text-amber-600 uppercase block">Mood Balance</span>
          <span className="text-base font-black text-amber-900 block">82% Positive</span>
          <span className="text-[9px] text-amber-700 font-bold block">Stress Reduction</span>
        </div>
      </div>
